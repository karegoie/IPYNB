...
def dotplot(blast_file, qbed, sbed, image_name, is_self=False,
    ...
    with open(blast_file, "r") as f:
        ...
        blasts = [BlastLine(line.strip()) for line in f]
    seen = set()
    ...
    for b in blasts:
        ...
        if bpscale:
            ...
            seen.add(key)
            qi, qstrand, q = qorder[query]
            si, sstrand, s = sorder[subject]
        strand = 'blue' if qstrand==sstrand else 'red'
        data.append((qi, si, strand))
    ...
    x, y, strand = zip(*data)
    ax.scatter(x, y, c=strand, s=1, lw=0, alpha=.9)
